...
def segment_clips():
  ...
  for file in os.listdir(DATA_PATH):
    ...
    if m is not None:
      ...
      command = f"ffmpeg -i {os.path.join(DATA_PATH, filename)} -f segment -segment_time {SEGMENT_TIME} {os.path.join(OUTPUT_PATH, fid + '_%03d.wav')}"
...
def plot_spectrogram(S, sr, fid, segment):
  ...
  path = os.path.join(SORTED_PATH, bird_name)
